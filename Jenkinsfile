pipeline {
    ...
    stages {
        ...
        stage("Build Docker Image") {
            steps {
                script {
                    env.IMAGE_TAG = "arru0057/amazon:${BUILD_NUMBER}"
            ...
            steps {
                script {
                    withCredentials([string(credentialsId: 'docker-cred', variable: 'dockerpwd')]) {
                        sh "docker login -u arru0057 -p ${dockerpwd}"
                        sh "docker tag amazon ${env.IMAGE_TAG}"
                        sh "docker push ${env.IMAGE_TAG}"

                        // Also push latest
                        sh "docker tag amazon arru0057/amazon:latest"
                        sh "docker push arru0057/amazon:latest"
                    }
                }
            }
        }
    ...
    always {
        script {
            ...
            emailext (
                ...
                """,
                to: 'parveezarman555@gmail.com',
                from: 'parveezarman555@gmail.com',
                mimeType: 'text/html',
